...
class Employee extends CI_Controller {
  ...
  public function onboardingForm($doubleEncodedParams)
	{ 
        ...
        if(isset($employeeData[0]['onboarding_status']) && $employeeData[0]['onboarding_status'] == 1){
        ...
        //   echo "<pre>"; print_r($data_user); exit;
			...
			if($updateEmployee){
			    ...
			    if($this->input->post('agree_terms_one') == '1'){
                 ...
                 $mailContent = $this->load->view('HR/Email/employeeCred',$mailData,TRUE); 
            $mail_from = $this->session->userdata('mail_from');
            $mail_protocol = $this->session->userdata('mail_protocol');
            $this->sendEmail($this->session->userdata('employeeEmail'),'BizAdmin - Welcome to HR management',$mailContent,$mail_from,'','',$mail_protocol);
