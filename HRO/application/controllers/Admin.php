     ...
     $menu_items = $this->display_menu();
       ...
       if(isset($user_id) && $user_id !='') {
		  ...
		  //   $data['name'] = $menu_items;
            ...
            $data['show_emp_rates_in_roster'] = $users[0]->show_emp_rates_in_roster;
        ...
        if ($this->form_validation->run() == true) {   
            ...
            $show_emp_rates_in_roster = $this->input->post('show_emp_rates_in_roster');
            $menus_list = $this->input->post('menus_list');
            // echo "<pre>"; print_r($menus_list);
           
        }
        if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email,$role,$phone,$branch,$menus_list,$supervisor,$show_emp_rates_in_roster)) {
                ...
                <span>=> Select Employee Portal</span><br></br>
                 <p><span><b>Login Details  </b></span></p>
                  <p><span>Username : '.$footscray_user->email.'</span></p>
                   <p><span>Password: <a href="http://aaria.com.au/HR/index.php/auth/forgot_password"> Reset your password here</a> </span></p>
		...
		if (!$this->ion_auth->logged_in()) {
		   ...
		   if($branch_id ==''){
			...
			$employees = $this->admin_model->get_employees_branchwise($branch_id,$type);
				...
				$this->load->view('general/header_general',$hdata);
				$this->load->view('employees/manage_employee',$data);
				// 	$this->load->view('employees/manageEmpLatest',$data);
				$this->load->view('general/footer');
		}
	}
