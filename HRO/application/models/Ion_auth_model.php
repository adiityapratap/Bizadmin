...
$check = $this->db->where($this->identity_column, $identity)
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $package, $phone = '', $additional_data = array(), $menu_access = array(),$supervisor,$show_emp_rates_in_roster)
	{  
		...
		$data = array(
		    ...
		    'show_emp_rates_in_roster'      => $show_emp_rates_in_roster,
...
public function emp_register($first_name,$last_name,$username, $password,$user_type,$active,$mobile,$res_id,$Specilized_category, $additional_data = array(), $groups = array())
	...
	public function login_employee($identity, $password, $remember=FALSE,$manager='')
	{
		...
		$this->db->select($this->identity_column . ',first_name,last_name, email,role, emp_id,password, status');
		                  $this->db->where('email', $this->db->escape_str($identity));
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
			 ...
			 $this->session->set_userdata('userlastname', $user_last_name);
			 $this->session->set_userdata('groupId', $group_id);
