...
<div class="main-content">
    ...
    <div class="page-content">
        <div class="RosterModel">
            <form method="post" id="rosterForm" action="<?php echo base_url(); ?>index.php/admin/update_complete_roster">
                ...
                <div class="modal-header py-2 px-3 bg-primary bg-gradient">
                    <div><h5 class="modal-title text-white" id="addRosterLabel">Update Roster</h5></div>
                    <div>
                        <button type="button" class="btn btn-success mr-1" onclick="rosterFormSubmit()">Update</button>
                              ...
                              <td class="gridjs-td hrsTag">
                                ...
                                <td class="gridjs-td hrsTag min-w100 <?php echo $week_days[$i]; ?>_col" rel="<?php echo $week_days[$i]; ?>" id="row_<?php echo $week_days[$i]; ?>_<?php echo $rowCount; ?>">
                                    ...
                                    <div class="approvedHrs pointerCursor" title="Edit Hours" rel="<?php echo $week_days[$i]; ?>" data-rel-id="row_<?php echo $week_days[$i]; ?>_<?php echo $rowCount; ?>" onclick="editRosterTime(this)" <?php if($row->$start_nameofday != 0 && $row->$end_nameofday != 0){ ?>style="display:block"<?php } else{ ?>style="display:none" <?php } ?>>
                                        ...
                                        <input type="hidden" class="startTimeInputValue" name="<?php echo $start_nameofday;  ?>[]" value="<?php if($row->$start_nameofday != '') echo date ('H:i',strtotime($row->$start_nameofday)); ?>">
                                        <input type="hidden" class="finishTimeInputValue" name="<?php echo $end_nameofday;  ?>[]" value="<?php if($row->$end_nameofday != '') echo date ('H:i',strtotime($row->$end_nameofday)); ?>">
                                        <input type="hidden" class="breakTimeInputValue" name="<?php echo $break_nameofday;  ?>[]" value="<?php echo $break_hrs; ?>">
                                        <input type="hidden" class="outletNameID" name="<?php echo $layout_nameofday;  ?>[]" value="<?php echo $row->$layout_nameofday; ?>">
...
function addRoster(){
    ...
    $(document).on("click", ".remove_field_button" , function() {
        var thisrow = $(this).closest('.employeeRole');
        var roster_id = $(this).closest('.employeeRole').find('.empInputs .roster_id').val();
      
        Swal.fire({
          title: "Deleting this employee will delete from the timesheet aswell if the timesheet is created already for this week. ",
          ...
          if (e.value) {
            ...
            $.ajax({
                ...
                url: "<?php echo base_url();?>index.php/admin/delete_single_roster",
                 data:'roster_id='+roster_id + '&id=' + roster_group_id,
        ...
        $(thisrow).closest('.employeeRole').remove();
