...
// 		else
	 ...
	 */
     ...
     $this->tenantDb->select('Global_users.*, Global_users_to_location.location_id');
     $this->tenantDb->from('Global_users');
     $this->tenantDb->join('Global_users_to_location', 'Global_users.id = Global_users_to_location.user_id');
     $this->tenantDb->where('Global_users.role_id !=', '4'); // because role_id = 4 is for employee , just for HR portal we are using that role
     $this->tenantDb->where('Global_users.is_deleted', 0);
      $this->tenantDb->where('Global_users.active', $isActive);
     $this->tenantDb->where('Global_users_to_location.location_id', $this->session->userdata('location_id'));
...
foreach ($selectedLocationIds as $locationId) {
	...
	public function update_last_login($id)
	{
		...
		$this->tenantDb->update($this->tables['users'], ['last_login' => time()], ['id' => $id]);
