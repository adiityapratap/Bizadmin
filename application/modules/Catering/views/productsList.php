                ...
                <div class="row align-items-center gy-3">
                    ...
                    <div class="col-sm-auto">
                        <div class="d-flex gap-1 flex-wrap">
                            <a href="#" class="btn btnAdd waves-effect waves-light shadow-none btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#newProductModal">
                                <i class="ri-add-line align-bottom me-1"></i> Add New Product
                            </a>
                            
                             <a href="#" class="btn btn-dark waves-effect waves-light shadow-none btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#newCatModal">
                        ...
                        <tbody class="list form-check-all">
                            <?php if (isset($listProducts) && !empty($listProducts)) { ?>
                                <?php foreach ($listProducts as $list) { ?>
                                    <tr data-id="<?php echo $list['product_id']; ?>">
                                        <td class="product_name"><?php echo $list['category_name']; ?></td>
                                        <td class="product_name"><?php echo $list['product_name']; ?></td>
                                        <td class="product_price"><?php echo number_format($list['product_price'],2); ?></td>
                            ...
                            data-productname="<?php echo $list['product_name']; ?>"
                            data-productprice="<?php echo $list['product_price']; ?>"
                            ...
                            <li class="list-inline-item" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Remove">
                                                    <a class="text-danger d-inline-block remove-item-btn" data-bs-toggle="modal" href="#deleteOrder" data-productid="<?php echo $list['product_id']; ?>">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
                    ...
                    <div class="form-group row mb-4">
                        ...
                        <div class="col-sm-9">
                            <select class="form-control" id="product_category_id" name="category_id"required>
                                ...
                                <?php foreach($listCategory as $listCat){ ?>
                                <option value="<?php echo $listCat['category_id'] ?>"><?php echo $listCat['category_name']; ?></option>
                                <?php }  ?>
                                <?php }  ?>
                                </select>
                        </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btnAdd loadingBtn btn-success btn-sm" id="product_add" onclick="add_new_product()">Add/Update</button>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="newCatModal" tabindex="-1" aria-labelledby="product-modal-title" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-dark btn-sm" data-bs-dismiss="modal">Close</button>
                <button type="button" class="btn btnAdd loadingBtn btn-success btn-sm"  id="category_add" onclick="add_new_category()">Add/Update</button>
            </div>
        </div>
    </div>
    <div id="loading" style="display:none;">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $('#searchInput').on('keyup', function () {
        ...
        $('#productTable').DataTable().search(searchValue).draw();
    });
...
$('#newProductModal').on('show.bs.modal', function (event) {
    ...
    if (buttontype === 'edit') {
        ...
        $('#newProductModal #new-product-name').val(productname);
        $('#newProductModal #product_category_id').val(categoryid);
        $('#newProductModal #product_id').val(productid);
        $('#newProductModal #new-product-price').val(productprice);
