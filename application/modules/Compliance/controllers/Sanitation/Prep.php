...
class Prep extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('Sanitation/prepList', $data);
    ...
    public function add()
    {
        if (isset($this->POST['prep_name'])) {
            ...
            $this->common_model->commonRecordCreate('Compliance_SanitationPrepArea', $prep_data);
            redirect('Compliance/Prep', 'refresh');
        } else {
            $data['form_type'] = 'add';
            $data['site_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_Sanitationsites', '', ['status' => 1, 'location_id' => $this->selected_location_id]);
            $this->load->view('general/header');
            $this->load->view('Sanitation/prepAdd', $data);
            $this->load->view('general/footer');
        }
    }
    ...
    public function edit($prep_id = '')
    {
        if (isset($this->POST['prep_name'])) {
            ...
            $this->common_model->commonRecordUpdate('Compliance_SanitationPrepArea', 'id', $prep_id, $prep_data);
            echo "success"; exit;
        } else {
            $data['prep_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_SanitationPrepArea', '', ['id' => $prep_id, 'location_id' => $this->selected_location_id]);
            $data['site_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_Sanitationsites', '', ['status' => 1, 'location_id' => $this->selected_location_id]);
            $data['form_type'] = 'edit';
            $this->load->view('general/header');
            $this->load->view('Sanitation/prepAdd', $data);
    ...
    public function change_status()
    {
        ...
        $this->common_model->commonRecordUpdate('Compliance_SanitationPrepArea', 'id', $this->POST['id'], $prep_data);
        echo "success";
    }
    ...
    public function updateSortOrder()
    {
        ...
        foreach ($newOrder as $index => $itemId) {
            ...
            $this->tenantDb->update('Compliance_SanitationPrepArea');
        }
        echo "success";
    }
