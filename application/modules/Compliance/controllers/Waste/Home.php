...
class Home extends MY_Controller {
    ...
    {   
        ...
        $data['site_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_WasteManagementsites',array(),'status=1'); 
        $data['prep_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_WasteManagementPrepArea',array(),'status=1'); 
  ...
  $data['todaysEnteredData'] = $todaysEnteredData;
    ...
    function history(){
      $data['site_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_WasteManagementsites', '', ['status' => 1]);
    ...
    public function historyData($encodedDateRange = '', $prep_id = '') {
        ...
        if (count($dateParts) == 2) {
            ...
            // Fetch data
            $data['site_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_WasteManagementsites', '', array()); // Adjust table name if needed
            $data['prep_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_WasteManagementPrepArea', '', array()); // Adjust table name if needed
            $condition = array('status' => 1);
            $data['products'] = $this->common_model->fetchRecordsDynamically('Compliance_wasteManagementproducts', '', $condition);
