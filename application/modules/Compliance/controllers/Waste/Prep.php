...
class Prep extends MY_Controller
{
    ...
    public function add()
    {
        if (isset($this->POST['prep_name'])) {
            ...
            $this->common_model->commonRecordCreate('Compliance_WasteManagementPrepArea', $prep_data);
            redirect('Compliance/Prep', 'refresh');
        } else {
            $data['form_type'] = 'add';
            $data['site_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_WasteManagementsites', '', ['status' => 1, 'location_id' => $this->selected_location_id]);
            $this->load->view('general/header');
            $this->load->view('WasteManagement/prepAdd', $data);
            $this->load->view('general/footer');
        }
    }
    ...
    public function change_status()
    {
        ...
        $this->common_model->commonRecordUpdate('Compliance_WasteManagementPrepArea', 'id', $this->POST['id'], $prep_data);
        echo "success";
    }
    ...
    public function updateSortOrder()
    {
        ...
        foreach ($newOrder as $index => $itemId) {
            ...
            $this->tenantDb->update('Compliance_WasteManagementPrepArea');
        }
        echo "success";
    }
