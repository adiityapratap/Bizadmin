...
class Task_model extends CI_Model{
	...
	function __construct() {
	   ...
	   public function add_task($data){
	    ...
	    return $this->tenantDb->insert('Compliance_tasks',$data);
		}
		
		public function fetchTaskList($id=''){
		 $sql = (isset($id) && $id !='' ? 'e.id='.$id.' AND ' : '');   
	    $query = $this->tenantDb->query("SELECT e.*,p.prep_name,s.site_name from Compliance_tasks e left join Compliance_prepArea p on e.prep_id = p.id  left join Compliance_sites s on p.site_id = s.id where ".$sql."  e.is_deleted = 0 AND  e.location_id = ".$this->selected_location_id." order by e.sort_order ASC");
	    if ($query !== false) {
	...
	public function getScheduledTasks() {
        ...
        $this->tenantDb->from('Compliance_tasks as Ct');
        $this->tenantDb->join('Compliance_prepArea as p', 'p.id = Ct.prep_id', 'left');
        $this->tenantDb->join('Compliance_sites as s', 's.id = p.site_id', 'left');
