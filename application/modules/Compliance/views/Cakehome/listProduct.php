                            ...
                            <div class="card py-3" id="orderList">
                                <div class="card-header  border-0 ">
                                    <div class="d-flex align-items-center gap-2">
                                        <h5 class="card-title mb-0 flex-grow-1 text-black">Cakes</h5>
...
function deleteThisRow(obj,rowId){
 ...
 $(document).on("click", "#delete-record" , function() {
     let id = $(this).val();
       $.ajax({
         type: "POST",
        url: "<?= base_url('General/updateTableStatus') ?>",
         data:'id='+id+'&table_name=Compliance_cakeproducts',
...
function editProduct(id) {
    $.get("<?= base_url('Compliance/Cake/Cakehome/getProductById/') ?>" + id, function(res) {
...
$('#productForm').on('submit', function(e) {
    e.preventDefault();
    $.ajax({
        url: "<?= base_url('Compliance/Cake/Cakehome/addOrUpdateProduct') ?>",
        type: "POST",
        data: $(this).serialize(),
        success: function(response) {
