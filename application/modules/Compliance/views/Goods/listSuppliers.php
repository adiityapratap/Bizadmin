            ...
            <div class="card py-3" id="orderList">
                <div class="card-header border-0">
                    <div class="d-flex align-items-center gap-2">
                        <h5 class="card-title mb-0 flex-grow-1 text-dark">Incoming Goods</h5>
                        <button type="button" class="btn btn-primary btn-sm fs-14" data-bs-toggle="modal" data-bs-target="#productModal" onclick="openProductModal()">
                            <i class="bi bi-plus-lg me-1"></i> Add Suppliers
                        </button>
                    </div>
                </div>
                    ...
                    <div class="table-responsive table-card mb-1">
                        <table class="table table-nowrap align-middle" id="activeRow">
                            ...
                            <tbody class="list form-check-all" id="sortable">
                                <?php if (!empty($suplliers)): ?>
                                    <?php foreach ($suplliers as $supllier): ?>
                                        <?php
                                        // Find matching prep area
                                        $selectedPrep = array_reduce($prep_detail, function ($carry, $prep) use ($supllier) {
                                            return $prep['id'] == $supllier['prep_id'] ? $prep : $carry;
                                        }, []);
                                        ...
                                        ?>
                                        <tr id="row_<?php echo htmlspecialchars($supllier['id']); ?>">
                                            <th scope="row">
                                                <div class="form-check">
                                                    <input class="form-check-input checkAll checkbox-item" type="checkbox" name="checkAll" value="<?php echo htmlspecialchars($supllier['id']); ?>">
                                                </div>
                                            </th>
                                            <td class="descr text-wrap handle"><?php echo htmlspecialchars($supllier['supplier_name']); ?></td>
                                            ...
                                            <td>
                                                         ...
                                                         <ul class="list-inline hstack gap-2 mb-0">
                                                                ...
                                                                <li class="list-inline-item edit" data-bs-toggle="tooltip" data-bs-trigger="hover"
                                                                    ...
                                                                    <a onclick="editsupplier(<?php echo htmlspecialchars($supllier['id']); ?>)"  class="text-primary d-inline-block edit-item-btn">
                                                                ...
                                                                <li class="list-inline-item" data-bs-toggle="tooltip" data-bs-trigger="hover"
                                                                    ...
                                                                    <a class="text-danger d-inline-block remove-item-btn" data-bs-toggle="modal"
                                                                        data-rel-id="<?php echo  $supllier['id']; ?>"  onclick="deleteThisRow(this, <?php echo htmlspecialchars($supllier['id']); ?>)">
...
<div class="modal fade" id="productModal" tabindex="-1" aria-labelledby="productModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form id="productForm" action="<?php echo base_url('Compliance/Goods/home/addOrUpdateProduct'); ?>" method="post">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="productModalLabel">Add/Edit Supplier</h5>
...
$(document).ready(function() {
    ...
    $("#sortable").sortable({
        ...
        update: function(event, ui) {
            ...
            $.ajax({
                url: "<?php echo base_url('Compliance/Goods/home/updateSortOrder'); ?>",
                type: "POST",
    ...
    window.openProductModal = function() {
        ...
        $('#supplier_id').val('');
        $('#productModalLabel').text('Add Supplier');
    ...
    window.editsupplier = function(id) {
        showLoading();
        $.ajax({
            url: "<?php echo base_url('Compliance/Goods/home/getProductById/'); ?>" + id,
            type: "GET",
            success: function(res) {
                ...
                try {
                    ...
                    $('#productForm').removeClass('was-validated');
                    $('#productModalLabel').text('Edit Supplier');
                    $('#productModal').modal('show');
                } catch (e) {
    ...
    $('#delete-record').on('click', function() {
        ...
        showLoading();
        $.ajax({
            url: "<?php echo base_url('Compliance/Goods/home/delete'); ?>",
            type: "POST",
            data: { id: id },
            success: function(response) {
                hideLoading();
                if (response === 'success') {
                    ...
                    showAlert('danger', 'Error deleting supplier.');
                }
            },
            error: function(xhr) {
                hideLoading();
                showAlert('danger', 'Error deleting supplier: ' + (xhr.responseJSON?.message || 'Unknown error.'));
            }
        });
    });
