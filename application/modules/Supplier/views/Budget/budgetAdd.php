                                ...
                                <div class="card card-animate">
                                    <div class="card-body">
                                        <div class="d-flex mb-3">
                                            ...
                                            <div class="flex-grow-1">
                                                <a href="javascript:void(0);" class="badge bg-success fs-14"> Budget Spent  </a>
                                            </div>
                                            <div class="flex-shrink-0 ">
                                                 <i class="ri-exchange-dollar-line fs-24" ></i>     
                                            </div>
                                        ...
                                        <div class="d-flex">
                                       <div class="flex-grow-1">     
                                        <h3 class="mb-2 text-black">$<span class="counter-value counter-valueWeeklySpent text-black" data-target="<?php echo (isset($locationBudgetDetails->weeklyBudgetSpent) ? $locationBudgetDetails->weeklyBudgetSpent : '') ?>"><?php echo (isset($locationBudgetDetails->weeklyBudgetSpent) ? $locationBudgetDetails->weeklyBudgetSpent : '') ?></span></h3>
                                        <input type="hidden" name="weeklyBudgetSpent" class="weeklyBudgetSpent" value="<?php echo (isset($locationBudgetDetails->weeklyBudgetSpent) ? $locationBudgetDetails->weeklyBudgetSpent : '') ?>" >
                                        <h6 class="text-black mb-0">Weekly</h6>
                                          </div> 
                                       <div class="flex-grow-1">     
                                        <h3 class="mb-2 text-black">$<span class="counter-value counter-valueMonthlySpent text-black" data-target="<?php echo (isset($locationBudgetDetails->monthlyBudgetSpent) ? $locationBudgetDetails->monthlyBudgetSpent : '') ?>"><?php echo (isset($locationBudgetDetails->monthlyBudgetSpent) ? $locationBudgetDetails->monthlyBudgetSpent : '') ?></span></h3>
                                         <input type="hidden" name="monthlyBudgetSpent" class="monthlyBudgetSpent" value="<?php echo (isset($locationBudgetDetails->monthlyBudgetSpent) ? $locationBudgetDetails->monthlyBudgetSpent : '') ?>">
                                        <h6 class="text-black mb-0">Monthly</h6>
                                          </div>       
                                         </div> 
                                    </div>
         ...
         <th>%</th>
                                      ...
                                      <tbody class="list main-category-row" >                                    
                                           <th colspan="10" class="text-black w-100" style="background-color: #dff0fa;"> <b><?php echo $mainCategoryName; ?></b></th>
                                                        ...
                                                        <?php if(isset($subCatDetails['noOfSuppliers']) && $subCatDetails['noOfSuppliers'] != 0) {  ?>
                                                       <?php
...
?>
<?php
$percentageSpentByCategory = 0;
if (!empty($subCatDetails['weeklyBudget'])) {
    $percentageSpentByCategory = ($subCatDetails['weeklySpent'] / $subCatDetails['weeklyBudget']) * 100;
}
$totalProgressColor = $percentageSpentByCategory <= 80 ? 'bg-success' : ($percentageSpentByCategory <= 100 ? 'bg-warning' : 'bg-danger');
?>
                                                ...
                                                <tr id="row_<?php echo  $subCatDetails['id']; ?>" class="parentRow">
                                                      ...
                                                      <input type="hidden" name="subCatId[]" value="<?php echo $subCatDetails['id']; ?>">
                                                       <td class="sub_category_name">
                                                        <?php echo (isset($subCatDetails['category_name']) ? $subCatDetails['category_name'] : ''); ?>
                                                        <?php if($percentageSpentByCategory > 0)  { ?>
                                                        <div class="progress mt-2" style="height: 10px;">
                        <div class="progress-bar <?php echo $totalProgressColor; ?>" role="progressbar" style="width: <?php echo $percentageSpentByCategory; ?>%;" aria-valuenow="<?php echo $percentageSpentByCategory; ?>" aria-valuemin="0" aria-valuemax="100">
                            <?php echo number_format($percentageSpentByCategory, 1) . '%'; ?>
                        </div>
                    </div>
                           <?php } ?>                            
    ...
    <input type="hidden" class="monthlyPercentage" name="monthlyPercentage_<?php echo $subCatDetails['id']; ?>" value="<?php echo ((isset($subCatDetails['monthlyPercentage']) ? $subCatDetails['monthlyPercentage'].'%' : '')); ?>">                                           
                                    ...
                                    <td>
                                <?php $classNameBtn = (isset($subCatDetails['noOfSuppliers']) && ($subCatDetails['noOfSuppliers'] > 1) == true ? '' : '') ?>        
                               <button class="btn btn-sm btn-success <?php echo $classNameBtn; ?>" type="button"  aria-controls="offcanvasRight" onclick="fetchSupplierFromSubCatID(this,<?php echo $subCatDetails['id']; ?>)">Suppliers</button>         
                                </td>   
                                     </tr>
                                                   <?php } ?>   
                                                   <?php } ?>
    ...
    <td class="totalWeeklyPercentage_<?php echo $mainCategoryId; ?>"><?php echo ((isset($totalWeeklyPercentage) ? floor($totalWeeklyPercentage).'%' : '')); ?></td> 
    ...
    <div class="form-icon">     
     ...
     <i class="bx bx-dollar"></i> 
                                      ...
                                      <td class="totalMonthlyPercentage_<?php echo $mainCategoryId; ?>"><?php echo ((isset($totalMonthlyPercentage) ? floor($totalMonthlyPercentage).'%' : '')); ?></td> 
       ...
       $(document).ready(function() {
          ...
          $('.mainCatTotalMonthly').each(function() {
            sumOfMainCatmonthly += parseFloat($(this).val()) || 0;
             ...
             let sumOfweekly = 0;
             $('.totalWeeklySpent').each(function() {
               // Remove $ and commas before parsing
             const value = $(this).text().replace(/[^0-9.-]+/g, '');
             sumOfweekly += parseFloat(value) || 0;
             });
             
             let sumOfMonthly = 0;
             $('.totalMonthlySpent').each(function() {
               // Remove $ and commas before parsing
             const value = $(this).text().replace(/[^0-9.-]+/g, '');
             sumOfMonthly += parseFloat(value) || 0;
             });
          ...
          $(".counter-valueWeeklySpent").html(sumOfweekly.toFixed(2));
          $(".weeklyBudgetSpent").val(sumOfweekly.toFixed(2));
          $(".counter-valueMonthlySpent").html(sumOfMonthly.toFixed(2));
          $(".monthlyBudgetSpent").val(sumOfMonthly.toFixed(2));
        ...
        //   $(".totalWeeklybudget_"+mainCatId).val(totalWeeklybudget.toFixed(2));
           ...
           function calculatePercentage(obj,totalBudget,type=''){
            //   var className = $('#exampleElement').attr('class');
             ...
             let monthlyPercentage = (currentRowMonthlybudget / (totalBudget*4)) * 100;
             ...
             let weeklyPercentage = (currentRowWeeklybudget / (totalBudget/4)) * 100;
             if(typeof weeklyPercentage != undefined){
             $(this).find(".weeklyPercentage").val(weeklyPercentage.toFixed(2)+'%');
             $(this).find(".weeklyPercentageTd").html(weeklyPercentage.toFixed(2)+'%');
             }
