<div class="main-content <?php  echo $isStockCountCompleted; ?>">
    <div class="page-content">
        <div class="container-fluid">
            <div class="alert alert-success shadow d-none" role="alert">
                <strong> Success !</strong> Order Sent Successfully.
            </div>
                        ...
                        <form id="orderForm">
                            <div class="row">
                                <div class="col-lg-4 mb-4">
                                    ...
                                    <input type="hidden" name="subcategory_id" value="<?php echo isset($selectedSupDetails[0]['category_id']) ? htmlspecialchars($selectedSupDetails[0]['category_id'], ENT_QUOTES) : ''; ?>">
                                    <select class="js-example-basic-single" required name="supplier_id" id="supplierNameInput" <?php echo $disabled; ?>>
                                        ...
                                        <?php if (!empty($suppliers_list)) {
                                            foreach ($suppliers_list as $sup) {
                                                echo '<option value="' . htmlspecialchars($sup['supplier_id'], ENT_QUOTES) . '">' . htmlspecialchars($sup['supplier_name'], ENT_QUOTES) . '</option>';
                                            }
                                        } ?>
                                    </select>
                                </div>
                                ...
                                <div class="col-lg-4 mt-4">
                                    <button type="button" class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#myModal">
                                        <i class="ri-add-line label-icon align-bottom fs-16 ms-2"></i> Add New Product
                                    </button>
                                    <a target="_blank" class="btn btn-outline-success d-none viewStockSummary">
                                        <i class="ri-bar-chart-2-fill label-icon align-bottom fs-16 ms-2 "></i> View Stock Summary
                                    </a>
                                </div>
    ...
    <div class="d-flex justify-content-end gap-4 flex-wrap">
        <span>
            ...
            <span class="minOrder">
                <?php
                $minOrderRaw = $selectedSupDetails[0]['min_order'] ?? 0;
                $minOrder = is_numeric($minOrderRaw) ? number_format((float)$minOrderRaw, 2, '.', ',') : '0.00';
                echo '$' . $minOrder;
                ?>
            </span>
        </span>
        ...
        <span>
            ...
            <span>
                <?php
                echo '$' . (is_numeric($budgetRemaining) ? number_format((float)$budgetRemaining, 2, '.', ',') : '0.00');
                ?>
            </span>
        </span>
    </div>
                            ...
                            <div class="row">
                                <div class="col-lg-9 mb-4 table-responsive">
                                   <table class="customtable table align-middle table-nowrap mb-0 table-bordered fs-12" id="productListTable">
    <thead class="table-light tableDynamicHeader">
        <tr>
            <th scope="col" style="width: 15%;">Product Code</th>
            <th scope="col" style="width: 45%;">Product Name</th>
            <th scope="col" style="width: 20%;">UOM</th>
            <th scope="col" style="width: 20%;">Price</th>
            <th scope="col" style="width: 20%;" class="stockTh">Order Quantity</th>
        </tr>
    </thead>
                                        ...
                                        <div class="card-body pt-2">
                                            <div class="table-responsive table-card">
                                                <table class="table table-borderless customtable align-middle mb-0">
                                                    <div class="heading">
                                                        <h6 class="text-black text-center">Delivery Info</h6>
                                                    </div>
                                                    <div class="cartProductSummary px-3">
                                                        <?php if (!empty($selectedSupDetails[0]['requireDD'])) { ?>
                                                            <div class="mb-3">
                                                                <label class="form-label">Delivery Date</label>
                                                                <input type="text" name="delivery_date" class="form-control flatpickr-input" data-provider="flatpickr" data-date-format="d M, Y" placeholder="Select date" readonly>
                                                            </div>
                                                        <?php } else { ?>
                                                            <div class="mb-3 requireDD">
                                                                <label class="form-label">Delivery Date</label>
                                                                <input type="text" name="delivery_date" class="form-control flatpickr-input" data-provider="flatpickr" data-date-format="d M, Y" placeholder="Select date" readonly>
                                                            </div>
                                                        <?php } ?>
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <textarea name="delivery_info" class="form-control delivery_info" placeholder="Delivery Info"><?php echo htmlspecialchars($selectedSupDetails[0]['delivery_info'] ?? '', ENT_QUOTES); ?></textarea>
                                                        </div>

                                                        <div class="mb-3">
                                                            <label class="form-label">Order Comments</label>
                                                            <textarea type="text" class="form-control" name="order_comments"></textarea>
                                                        </div>

                                                        <div class="mb-3">
                                                            <label class="form-label">Email</label>
                                                            <input type="text" class="form-control supplier_email" name="supplier_email" value="<?php echo htmlspecialchars($selectedSupDetails[0]['email'] ?? '', ENT_QUOTES); ?>">
                                                        </div>

                                                        <div class="mb-3">
                                                            <label class="form-label">Cc Email</label>
                                                            <input type="text" class="form-control supplier_CCemail" name="supplier_CCemail" value="<?php echo htmlspecialchars($selectedSupDetails[0]['cc'] ?? '', ENT_QUOTES); ?>">
                                                        </div>
                                                    </div>
                                                    ...
                                                    <tfoot>
                                                        <tr class="table-active">
                                                            ...
                                                            <th colspan="1">Total (AUD) : <span class="fw-semibold cartTotalAmount"></span></th>
                                        ...
                                        <table class="table table-bordered align-middle mb-0 customtable" id="newProductTable">
                                            ...
                                            <tbody class="prdctWithZeroOrderQty">
                                                            ...
                                                            <div class="modal-footer">
                                                                <button type="button" class="btn btn-success btn-sm addToCartFromModal" data-bs-dismiss="modal">Add to cart</button>
...
$(document).ready(function () {
 ...
 if(((suppDetails[0]?.requireSC == "1" || suppDetails[0]?.requirePL == "1") && parseInt(orderQty) > 0) || (suppDetails[0]?.requireSC == "0" || suppDetails[0]?.requirePL == "0") || (suppDetails[0]?.requireSC == "1" && suppDetails[0]?.requirePL == "0") || (isMonthlyStockOrder == 1)){
  ...
  htmlC += '<td scope="row" class="product_name text-wrap"><input type="hidden" class="product_price" value="' + (value?.price || 0) + '"><input type="hidden" name="product_id[]" value="' + (value?.product_id || '') + '">' + (value?.product_name || '') + '</td>';
  htmlC += '<td scope="row" class="product_uom"><input type="hidden" name="product_uom_id[]"  value="' + (value?.product_uom_id || '') + '">' + uomName + '</td>';
  htmlC +='<td scope="row" class="productPriceTd"><input type="hidden" name="product_unit_price[]"  value="' + (value?.price || '') + '"> $' + (value?.price || '') + '</td>';
  ...
  htmlD += '<td scope="row" class="product_name text-wrap"><input type="hidden" class="product_price" value="' + (value?.price || 0) + '"><input type="hidden" name="product_id[]" value="' + (value?.product_id || '') + '">' + (value?.product_name || '') + '</td>';
  htmlD += '<td scope="row" class="product_uom"><input type="hidden" name="product_uom_id[]"  value="' + (value?.product_uom_id || '') + '">' + uomName + '</td>';
  htmlD +='<td scope="row" class="productPriceTd"><input type="hidden" name="product_unit_price[]"  value="' + (value?.price || '') + '"> $' + (value?.price || '') + '</td>';
...
$(".order_total").val(orderTotal.toFixed(2));
    ...
    row.find('.remove-item-btn').replaceWith(''); 
    ...
    $(".cartTotalAmount").html('$'+updatedCartTotal.toFixed(2))
     $(".orderTotal").val(updatedCartTotal.toFixed(2));
      ...
      // Add to Cart from Modal Footer
            $('.addToCartFromModal').on('click', function (e) {
                e.preventDefault();
                productListTable.clear();
                deletedProductListTable.clear();
                ...
                $('.prdctWithZeroOrderQty tr').each(function () {
                    let row = $(this);
                    let qty = parseInt(row.find('.product-quantity').val()) || 0;
                    if (qty > 0) {
                        row.find('.plusNew').removeClass('plusNew').addClass('plus');
                        row.find('.minusNew').removeClass('minusNew').addClass('minus');
                        let itemPrice = parseFloat(row.find('.itemPrice').val()) || 0;
                ...
                let cartTotal = parseFloat($(".cartTotalAmount").html().replace(/\$/g, '')) || 0;
                let updatedCartTotal = cartTotal + totalItemPrice;
                $(".cartTotalAmount").html('$' + updatedCartTotal.toFixed(2));
                $(".order_total").val(updatedCartTotal.toFixed(2));
...
function sendOrder(){
  ...
  <?php if (isset($isStockCountCompleted) && $isStockCountCompleted == 0): ?>
<script>
    $(document).ready(function () {
         $("#myModal").modal('show');
         
            // Option 1: Focus the first .product-quantity input
            let firstInput = document.querySelector('.product-quantity');
            if (firstInput) {
                firstInput.focus();
                firstInput.scrollIntoView({ behavior: 'smooth', block: 'center' });
            } else {
                console.warn('No .product-quantity input found');
            }
