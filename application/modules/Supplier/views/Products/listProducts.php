...
<div class="main-content">
        ...
        <div class="d-flex justify-content-sm-end gap-2">
         <a href="/Supplier/list"  class="btn bg-orange waves-effect btn-label waves-light btn-sm"><i class="ri-reply-fill label-icon align-middle fs-16 me-2"></i><span>Back</span></a>                                            
        ...
        <a href="/Supplier/Product/downloadSampleProduct/<?php echo $supplier_id; ?>"  class="btn bg-warning btn-sm waves-effect btn-label waves-light"><i class="ri-file-download-fill label-icon align-middle fs-16 me-2"></i><span>Download Sample</span></a>                                                          
                                          ...
                                          <table class="table align-middle table-nowrap" id="customerTable">
                                                    <thead class="table-light">
                                                       <tr>
                                                            ...
                                                            <th class="sort" data-sort="parLevel">PAR </th>
                        ...
                        <td class="product_code">
                                            <input class="form-input form-control-icon field-input" 
                                                   name="product_code_<?php echo htmlspecialchars($row['product_id'], ENT_QUOTES); ?>" 
                                                   value="<?php echo htmlspecialchars($row['product_code'], ENT_QUOTES); ?>" 
                                                   data-product-id="<?php echo htmlspecialchars($row['product_id'], ENT_QUOTES); ?>" 
                                                   data-field-name="product_code">
                                        </td>
                                        <td class="product_name">
                                            <input class="form-input form-control-icon field-input" 
                                                   name="product_name_<?php echo htmlspecialchars($row['product_id'], ENT_QUOTES); ?>" 
                                                   value="<?php echo htmlspecialchars($row['product_name'], ENT_QUOTES); ?>" 
                                                   data-product-id="<?php echo htmlspecialchars($row['product_id'], ENT_QUOTES); ?>" 
                                        ...
                                        <td class="price">
                                            <div class="form-icon">
                                                <input class="form-input form-control-icon field-input" 
                                                       name="price_<?php echo htmlspecialchars($row['product_id'], ENT_QUOTES); ?>" 
                                        ...
                                        <td class="PARLevelQty">
                                            <input class="form-input form-control-icon field-input" 
                                                   name="par_level_qty_<?php echo htmlspecialchars($row['product_id'], ENT_QUOTES); ?>" 
                                                   value="<?php echo htmlspecialchars($row['PARLevelQty'], ENT_QUOTES); ?>" 
                                                   data-product-id="<?php echo htmlspecialchars($row['product_id'], ENT_QUOTES); ?>" 
                                                   data-field-name="PARLevelQty">
                                        </td>
                                       ...
                                       <table class="table align-middle table-nowrap" id="customerTable">
                                                    <thead class="table-light">
                                                        <tr>
                                                            ...
                                                            <th class="sort" data-sort="price">Price</th>
                                                             <th class="sort" data-sort="price">PAR</th>
                                                             <th class="sort" data-sort="price">Category</th>
                                                            <th class="sort" data-sort="approve">Status</th>
                                                            <th class="no-sort" width="200">Action</th>
                                                            </tr>
                                                     ...
                                                     <tbody class="list form-check-all">
                                                        ...
                                                        <tr id="row_<?php echo  $row['product_id']; ?>" >
                                                            ...
                                                            <td class="price"><?php echo "$".number_format($row['PARLevelQty'],2); ?></td>
                                                            <td class="category"><?php echo $row['product_category_name']; ?></td>
    ...
    $("#sortable").sortable({
        ...
        update: function(event, ui) {
         ...
         <script>
        $(document).ready(function () {
            // Handle blur event on all editable fields like price,parlevel etc
            $(document).on('blur', '.field-input', function () {
                let $input = $(this);
                let product_id = $input.data('product-id');
                let field_name = $input.data('field-name');
                let value = $input.val().trim();

                // Validate input
                if (!product_id || !field_name || value === '') {
                    $('#errorAlert').html('<strong>Error!</strong> Invalid input or field name.').removeClass('d-none').fadeIn();
                    setTimeout(() => $('#errorAlert').addClass('d-none'), 3000);
                    return;
                }

                // Additional validation for price (must be a non-negative number)
                if (field_name === 'price' && (isNaN(value) || parseFloat(value) < 0)) {
                    $('#errorAlert').html('<strong>Error!</strong> Price must be a valid non-negative number.').removeClass('d-none').fadeIn();
                    setTimeout(() => $('#errorAlert').addClass('d-none'), 3000);
                    return;
                }

                // Additional validation for PARLevelQty (must be a non-negative integer)
                if (field_name === 'PARLevelQty' && (isNaN(value) || parseInt(value) < 0 || !Number.isInteger(parseFloat(value)))) {
                    $('#errorAlert').html('<strong>Error!</strong> PAR Level Quantity must be a valid non-negative integer.').removeClass('d-none').fadeIn();
                    setTimeout(() => $('#errorAlert').addClass('d-none'), 3000);
                    return;
                }
                ...
                let csrf_hash = '<?php echo $this->security->get_csrf_hash(); ?>';
                ...
                $.ajax({
                    ...
                    url: "/Supplier/update_product_info",
                    data: data,
                    dataType: 'json',
                    success: function (response) {
                        if (response.status === 'success') {
                            $('#successAlert').html('<strong>Success!</strong> ' + response.message).removeClass('d-none').fadeIn();
                            setTimeout(() => $('#successAlert').addClass('d-none'), 3000);
                            // Update CSRF token
                            if (response.csrf_hash) {
                                $('[name="' + csrf_name + '"]').val(response.csrf_hash);
                            }
                            // Update input value formatting (e.g., price to 2 decimals)
                            if (field_name === 'price') {
                                $input.val(parseFloat(value).toFixed(2));
                            }
                        } else {
                            $('#errorAlert').html('<strong>Error!</strong> ' + (response.message || 'Failed to update field.')).removeClass('d-none').fadeIn();
                            setTimeout(() => $('#errorAlert').addClass('d-none'), 3000);
                        }
                    },
                    error: function (xhr, status, error) {
                        console.error('AJAX Error:', xhr, status, error);
                        $('#errorAlert').removeClass('d-none').fadeIn();
                        setTimeout(() => $('#errorAlert').addClass('d-none'), 3000);
                    }
                });
            });
