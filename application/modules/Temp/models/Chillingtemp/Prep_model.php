...
class Prep_model extends CI_Model{
	...
	function __construct() {
	   ...
	   public function add_site($data){
	    ...
	    return $this->tenantDb->insert('TEMP_chillingPrepArea',$data);
		}
  ...
  function fetchAllPrepArea(){
      $query = $this->tenantDb->query("SELECT tp.*,ts.site_name from TEMP_chillingPrepArea tp  left join TEMP_chillingSites ts on tp.site_id = ts.id   where tp.is_deleted = 0 AND  tp.location_id = ".$this->selected_location_id." order by tp.sort_order ASC");
	    if ($query !== false) {
