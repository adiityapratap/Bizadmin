...
class Slicingtemp_model extends CI_Model {
    ...
    $this->tenantDb->select("
        ...
        (
            ...
            FROM TEMP_slicingPrepArea p
            WHERE p.site_id = s.id
              AND p.is_deleted = 0
        ) AS prep_areas
    ", false);

    $this->tenantDb->from('TEMP_slicingSites s');
    $this->tenantDb->where('s.location_id', $this->selected_location_id);
    $this->tenantDb->where('s.is_deleted', 0);
    $this->tenantDb->where('s.status', 1);

    if ($siteId != '') {
        $this->tenantDb->where('s.id', $siteId);
    }
    ...
    public function fetchExceededTempData() {
        $this->tenantDb->select('TEMP_slicingTemprecordHistory.*, TEMP_slicingSites.site_name, TEMP_slicingPrepArea.prep_name, TEMP_slicingSites.manager_comments');
        $this->tenantDb->from('TEMP_slicingTemprecordHistory');
        $this->tenantDb->join('TEMP_slicingSites', 'TEMP_slicingTemprecordHistory.site_id = TEMP_slicingSites.id');
        $this->tenantDb->join('TEMP_slicingPrepArea', 'TEMP_slicingTemprecordHistory.prep_id = TEMP_slicingPrepArea.id');
        $this->tenantDb->where('TEMP_slicingTemprecordHistory.date_entered >=', date('Y-m-d', strtotime('-7 days')));
        $this->tenantDb->where('TEMP_slicingTemprecordHistory.date_entered <=', date('Y-m-d'));
        $this->tenantDb->where('TEMP_slicingTemprecordHistory.location_id', $this->selected_location_id);
        $this->tenantDb->where('TEMP_slicingTemprecordHistory.isTempOk', 'notOk');
        $this->tenantDb->where('TEMP_slicingPrepArea.status', 1);
        $this->tenantDb->where('TEMP_slicingSites.status', 1);
        $query = $this->tenantDb->get();
        return $query->result_array();
    }
  ...
  public function fetchTempViewHistoryData($fromDate, $toDate, $site_id) {
    ...
    $this->tenantDb->select('
        ...
        h.product_id,
        p.product_name,
        h.internal_batch_code_allocated,
        h.start_slicing,
        h.time_finished_slicing,
        h.temp_of_product_at_end_of_slicing,
        h.time_chilling_process_started,
        h.time_chilling_process_finished,
        h.temp_of_product_at_start_of_slicing,
        h.comments,
        h.entered_by,
        h.signature,
        h.is_completed,
